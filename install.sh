...
# Always pull the latest docker image
ExecStartPre=/usr/bin/docker pull baseboxorg/zend:latest
ExecStart=/usr/bin/docker run --rm --net=host -p 9033:9033 -p 18231:18231 -v /mnt/zen:/mnt/zen --name zen-node baseboxorg/zend:latest
[Install]
WantedBy=multi-user.target
EOF
...
# Always pull the latest docker image
ExecStartPre=/usr/bin/docker pull baseboxorg/secnodetracker:latest
#ExecStart=/usr/bin/docker run --init --rm --net=host -v /mnt/zen:/mnt/zen --name zen-secnodetracker baseboxorg/secnodetracker:latest
ExecStart=/usr/bin/docker run --rm --net=host -v /mnt/zen:/mnt/zen --name zen-secnodetracker baseboxorg/secnodetracker:latest
[Install]
WantedBy=multi-user.target
EOF
